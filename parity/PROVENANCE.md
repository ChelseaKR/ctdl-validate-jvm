...
`parity/fixtures/` holds 30 CTDL JSON-LD payloads. They are synthetic. Every
...
Nineteen were written for this repository, because the sibling's fixtures
...
| `concept_scheme_membership.json` | Checks 7 to 9, on one certificate: a concept in the named scheme (silent), one from another scheme (`CONCEPT_OUTSIDE_SCHEME`), one the snapshot does not declare (`CONCEPT_OUTSIDE_SNAPSHOT`), an alignment object naming its term only in words (`CONCEPT_NOT_IDENTIFIED`), a term written directly rather than wrapped, an unstable concept (`TERM_UNSTABLE`), and bare literals on language-map properties (`LANGUAGE_MAP_EXPECTED`) beside a real language map |
| `envelope_graph_uri_mismatch.json` | the `@graph` envelope's own `@id` naming a CTID no entity in the payload declares (`CTID_URI_MISMATCH`, on the envelope) |
| `envelope_graph_uri_malformed.json` | the envelope's graph URI with a bare UUID tail (`REGISTRY_URI_MALFORMED`, saying the URI is the envelope's own) |
| `nothing_checked.json` | a document with no `ceterms:` or `ceasn:` term in it: no findings, and a text report that ends by saying nothing was checked, in the singular |
| `repeated_identifier_merge.json` | Check 6: one `@id` declared inline first and at the top level second, read as one entity typed with the union of both, its values de-duplicated by Python's `==` -- `true` absorbs `1` and `2.0` absorbs `2`, which Jackson's equality would keep apart |

Against the pinned `0.2.1` the last five expect none of checks 6 to 9's
findings, because that release has none of those checks. What they are compared
against is described under "Against the reference's unreleased main" below.
...
Measured 2026-09-11, after checks 6 to 9 were ported, with the reference at
`origin/main` = `816591ac6364`:

| reference ref | its validation rules | this port implements | behind by | ahead by |
|---|---:|---|---:|---:|
| `v0.1.0` | 19 | 19 of 19 | 0 | 9 |
| `v0.2.1` (the pin, newest release) | 20 | 20 of 20 | 0 | 8 |
| `origin/main` | 28 | **28 of 28** | 0 | 0 |

Before `--resolve` was ported the same three rows read 19 of 19, 19 of 20 and
21 of 28; after it, 19 of 19, 20 of 20 and 22 of 28.
...
and the agreement is why that suite is red on this branch.**
...
the same eight: `CONCEPT_RANGE_CONFLICT`, `VERSION_RANGE_CONFLICT`, and the
six codes of checks 6 to 9. The census declares the first two and fails on the
other six, which is ADR 0004 doing its job: a rule no release carries may not be
emitted ahead of the pin. Two readers naming the same eight from two data paths
is the strongest evidence available that either of them is right.
...
**The six are ported, and none of them is in a release.**
...
to 9, all landed on `main` after `0.2.1`. The pin cannot move to them until the
reference cuts a release carrying them, and until it does this port is ahead of
the pin by exactly these six. `REF_RESOLVED_SUPPLIED` was the seventh and is on
`main`.
...
as covering 58% of the reference rather than 100%, and would move whenever the
...
### Against the reference's unreleased main

Byte equality with the pin cannot show checks 6 to 9, because the pin has none
of them. So `tools/next_release_parity.py` makes the comparison `ParityTest`
makes, over both corpora and every `--resolve` directory, against the reference
at the one commit `parity/reference-main-commit.txt` names -- imported from a
clean checkout of that commit, never from whatever is installed -- and a
non-gating workflow runs it on every pull request.

Measured 2026-09-11 at `816591ac6364`: **35 of 35 parity documents byte-equal.**
Against the pin, `ParityTest` is red on exactly the 11 fixtures whose 0.2.1 and
`main` documents differ, and on no other.

The harness, its commit file and its workflow are temporary. When the pin
reaches a release carrying these rules, `parity/expected/` becomes this
comparison, and all three are deleted rather than kept as a second copy of it.
[ADR 0008](../docs/adr/0008-checks-6-to-9-are-ported-before-their-release.md)
records the arrangement and the steps that bump takes.
