...
landed a rule change on `main` — the concept-range disposition — which this
port carries and no released version does. Bumping the pin is a review of a
rule-set change, not a dependency chore, so it has not been done reflexively;
see the ahead corpus below for how the resulting divergence is bounded in the
meantime.
